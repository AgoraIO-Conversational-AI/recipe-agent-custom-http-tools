...
const compileTargets = ["server.py", "agent.py", "http_tools.py"]
const requiredEnv = ["AGORA_APP_ID", "AGORA_APP_CERTIFICATE", "HTTP_TOOLS_BASE_URL", "HTTP_TOOLS_API_KEY"]
const publicUrlKey = "HTTP_TOOLS_BASE_URL"
const requirements = ["requirements.txt", "requirements-dev.txt"]
const setupMessage = "\nSetup complete! Next steps:\n   1. agora project env write server/.env.local   (or fill it in manually)\n   2. ngrok http 8000                            (expose /tools to Agora Engine)\n   3. Add HTTP_TOOLS_BASE_URL=<https-tunnel-url> and a random HTTP_TOOLS_API_KEY\n   4. bun run dev\n\n"
...
function doctor(local = false) {
  ...
  if (publicUrlKey) {
    ...
    if (!value) throw new Error(`${publicUrlKey} is missing in server/.env.local`)
    let parsed: URL
    try {
      parsed = new URL(value.trim())
    } catch {
      throw new Error(`${publicUrlKey} must be a valid public HTTPS URL.`)
    }
    if (parsed.protocol !== 'https:') throw new Error(`${publicUrlKey} must use HTTPS.`)
    const hostname = parsed.hostname.toLowerCase()
    if (['localhost', '127.0.0.1', '::1', '[::1]'].includes(hostname)) {
      throw new Error(`${publicUrlKey} must be publicly reachable by Agora Engine.`)
    }
  }
}
